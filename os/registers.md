...
| Claude OS console with Jarvis | https://claude.ai/code/artifact/2a9b7e57-dbc5-49e3-a4d7-c0a36bd236b2 | Sessions, on request; source at `os/claude-os-overview.html`. Daily console: live Today tab (viewer's "Salesforce Production" and "Asana" connectors), Jarvis HUD assistant (sample capability, viewer's own Claude usage). Write surface, each behind an in-page Approve card and verified by re-read: Case internal note, public reply, close with reason (via `createSobjectRecord`/`updateSobjectRecord`), Asana task comment and complete (via `add_comment`/`update_tasks`). Reads via `soqlQuery`/`search_tasks`. No bulk actions, no reassignment, no email from the page. Update this row and the page when the registers change materially |
