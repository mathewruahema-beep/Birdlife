...
| The BirdLife Brain (human brain map) | https://claude.ai/code/artifact/7c0faca1-812a-4ce2-8b0e-b9d501ef95d3 | Reads itself: nothing is baked in. Source at `os/birdlife-brain.html`, first published 12 Sep 2026, made live and reshaped as a human brain the same day. At every open the page reads the repository through the viewer's claude.ai "GitHub" connector (`get_file_contents`, READ ONLY, the only tool declared; the credential behind the connector must be able to see the private Birdlife repo): CLAUDE.md (skill table), INDEX.md, `os/registers.md` (routine, connector and artefact registers), the `os/`, `.claude/skills/`, `memory/journal/`, `memory/patterns/` and decisions folders. Layout is a side-view brain with each layer in the lobe whose job it mirrors: kernel at the thalamus, doctrine and OS files in the basal ganglia ring, ADRs prefrontal, cross-cutting skills frontal, system skills parietal, deep dives parietal-rear, connector skills insula, memory temporal, artefacts occipital, routines cerebellum, live systems as nerves from the brainstem, human copy outside the skull. In-use model: a node rings when the page is reading it (each connector call is logged in the Activity panel with Melbourne time) or, for routines, when its register schedule says it is due (plus or minus 10 minutes) or fired within the hour; today's journal shows as written today. Only GitHub is exercised by the page; other connectors show as connected per the register, not observed. Branch field defaults to the working branch `claude/birdlife-brain-build-02zobt`; change it to `main` once that branch exists. Refreshes every five minutes while open; Reload re-reads. Click a node to read the file's heading; Diagnose in the banner prints permission state, reachable connectors and per-read outcomes. Skill-to-system edges are a static map in the page (update when a skill starts reading a new system); routine edges are keyword mentions from the register rows. If the register table formats change (section 1 Active recurring and One-shots tables, section 3 bold connector names, section 4 table), the page's parsers need the same change |
