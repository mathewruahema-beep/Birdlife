...
| The BirdLife Brain (VIKI-style map) | https://claude.ai/code/artifact/7c0faca1-812a-4ce2-8b0e-b9d501ef95d3 | Sessions, on request ("rebuild the brain map"); source at `os/birdlife-brain.html`, first published 12 Sep 2026 from commit 8036c72. Static radial neural map of the brain: CLAUDE.md at the centre, then INDEX, birdlife-core, OS rules, registers, lenses, console and memory rules, then the 33 skills, then journals, patterns, 13 routines and 21 ADRs, then 19 live systems, 6 artefacts and the 3 human-copy locations, 13 colour-coded layers, edges labelled by what reads what. Read-only: no connectors, no capabilities, no writes; data is baked in at build time, so it goes stale as the registers change. Rebuild when a skill, routine, ADR or artefact is added or retired |
