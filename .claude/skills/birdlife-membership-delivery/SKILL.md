...
| When | What | Where |
|---|---|---|
| Mon/Wed/Fri 7am | Delivery briefing routine `trig_01F7wy1pGcauqCsgUxVU4b47` (read only) pushes the briefing to Mathew's phone; Fridays it adds the status draft. It cannot publish artifacts unattended, so the Desk's briefing block is filled only by interactive sessions | routine, see `routines/membership-delivery-briefing.md` |
